...
// Diagnostic endpoint to verify routes are registered
  ...
  try {
    ...
    for (const model of modelsToTry) {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        ...
        if (providerMessage.length > 300) providerMessage = providerMessage.slice(0, 300) + 'GÇª';
    ...
    try {
      ...
      if (raw) {
        ...
        if (shortRateLimitMsg.length > 300) shortRateLimitMsg = shortRateLimitMsg.slice(0, 300) + 'GÇª';
      }
    } catch (e) {
    }
...
wait_for_apt_lock() {
  ...
  while fuser /var/lib/dpkg/lock /var/lib/apt/lists/lock /var/lib/dpkg/lock-frontend /var/cache/apt/archives/lock >/dev/null 2>&1; do
    if [ "$waited" -ge "$max_wait" ]; then
      ui_line "apt lock held too long GÇö force-clearing..."
