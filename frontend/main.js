...
function getUserDataPaths() {
    ...
    return {
        ...
        AlphaAIHistoryPath: path.join(base, 'AlphaAI-history.jsonl'),
        AlphaAIAuditPath: path.join(base, 'AlphaAI-audit.jsonl'),
    };
}
...
// --- SECURITY / AlphaAI SETTINGS + LOCAL STORAGE ---
...
ipcMain.handle('security:set-settings', async (event, nextSettings) => {
    try {
        ...
        const { AlphaAIHistoryPath, AlphaAIAuditPath } = getUserDataPaths();
        applyRetention(AlphaAIHistoryPath, saved?.AlphaAI?.retentionDays);
        applyRetention(AlphaAIAuditPath, saved?.AlphaAI?.retentionDays);
...
ipcMain.handle('AlphaAI:history-append', async (event, entry) => {
    try {
        const settings = loadSecuritySettings();
        const { AlphaAIHistoryPath } = getUserDataPaths();
        jsonlAppend(AlphaAIHistoryPath, {
            ts: new Date().toISOString(),
            ...((entry && typeof entry === 'object') ? entry : { text: String(entry ?? '') }),
        });
        applyRetention(AlphaAIHistoryPath, settings?.AlphaAI?.retentionDays);
        return { success: true };
    } catch (e) {
        return { success: false, error: e.message };
    }
...
ipcMain.handle('AlphaAI:audit-append', async (event, entry) => {
    try {
        const settings = loadSecuritySettings();
        const { AlphaAIAuditPath } = getUserDataPaths();
        jsonlAppend(AlphaAIAuditPath, {
            ts: new Date().toISOString(),
            ...((entry && typeof entry === 'object') ? entry : { message: String(entry ?? '') }),
        });
        applyRetention(AlphaAIAuditPath, settings?.AlphaAI?.retentionDays);
        return { success: true };
    } catch (e) {
        return { success: false, error: e.message };
    }
...
ipcMain.handle('AlphaAI:history-export', async () => {
    try {
        const { AlphaAIHistoryPath } = getUserDataPaths();
        const data = jsonlReadAll(AlphaAIHistoryPath, 20000);
...
ipcMain.handle('AlphaAI:audit-export', async () => {
    try {
        const { AlphaAIAuditPath } = getUserDataPaths();
        const data = jsonlReadAll(AlphaAIAuditPath, 20000);
        return { success: true, data };
    } catch (e) {
        return { success: false, error: e.message };
    }
...
ipcMain.handle('ssh:ai-command', async (event, { prompt, serverContext, chatHistory, mode }) => {
    ...
    try {
        ...
        if (useMode === 'chat') {
            ...
            effectivePrompt += `\n\nSYSTEM INSTRUCTION: As AlphaAI, if asked to create, modify, or update a file, ALWAYS use the current user's default folder (e.g., ~/) unless specifically asked to put it in the root directory (/). Do not use /root/ or / unless explicitly instructed by the user. Modify/Read files with appropriate permissions.`;
